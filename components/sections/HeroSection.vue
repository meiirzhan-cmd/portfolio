<template>
  <section
    class="relative min-h-screen flex items-center pt-24 pb-8 overflow-hidden"
  >
    ...
    <div class="absolute inset-0 z-0 overflow-hidden">
      <div
        ...
        style="
          background-image: linear-gradient(135deg, #0f2051, #204f9b, #4c209b);
        "
      ></div>
      <!-- Background particles/shapes -->
      <div class="absolute inset-0">
        <div
          class="absolute top-1/4 left-1/4 w-64 h-64 rounded-full bg-primary-600/20 blur-3xl animate-pulse"
        ></div>
        <div
          class="absolute bottom-1/3 right-1/4 w-80 h-80 rounded-full bg-secondary-600/20 blur-3xl animate-pulse"
      ...
      <div class="grid grid-cols-1 lg:grid-cols-5 gap-8 md:gap-12 items-center">
        ...
        <SectionTransition class="slide-right lg:col-span-3">
          <div class="space-y-8">
            ...
            <h1
              class="text-4xl md:text-5xl xl:text-6xl font-bold text-white leading-tight"
            >
              <span
                class="bg-clip-text text-transparent bg-gradient-to-r from-primary-300 to-secondary-300"
              >
            ...
            <h2
              class="text-xl md:text-2xl text-primary-200 font-medium relative inline-block"
            >
              {{ translate(profile.title) }}
              <span
                class="absolute bottom-0 left-0 w-full h-0.5 bg-gradient-to-r from-primary-400 to-secondary-400 transform scale-x-0 origin-left transition-transform duration-1000 animate-slide-in"
              ></span>
            </h2>
        ...
        >
          <div class="relative">
            ...
            <div class="absolute inset-0 -m-6 rotate-45 opacity-70">
              <div
                class="absolute inset-0 border-2 border-primary-400/30 rounded-full animate-spin-slow"
              ></div>
            </div>
            <div class="absolute inset-0 -m-12 -rotate-45 opacity-70">
              <div
                class="absolute inset-0 border-2 border-secondary-400/30 rounded-full animate-spin-slow-reverse"
              ></div>
            </div>
            ...
            <div class="animated-profile">
              <div class="profile-content">
                ...
                />
                <h2>
                  {{ translate(profile.name) }}<br /><span>{{
                    translate(profile.title)
                  }}</span>
                </h2>
                <NuxtLink
            ...
            <div
              class="absolute -bottom-4 -right-4 w-3 h-3 rounded-full bg-primary-400 animate-pulse"
            ></div>
            <div
